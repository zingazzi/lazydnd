	...
	if m.ActivePanel == DiceRoller {
		...
		if m.MonsterSearchMode {
			...
			if len(m.MonsterSuggestions) > 0 && m.MonsterSuggestionIndex >= 0 && m.MonsterSuggestionIndex < len(m.MonsterSuggestions) {
				...
				// If we're adding a monster to an encounter, do that now
				if m.AddingMonsterToEncounter {
					m = AddMonsterToEncounter(m, selectedMonsterName)
					m.AddingMonsterToEncounter = false
				}
			}
		} else {
